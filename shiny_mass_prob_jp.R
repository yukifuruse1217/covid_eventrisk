...
ui <- fluidPage(
  ...
  # title
  titlePanel("新型コロナの感染状況にもとづくイベント開催リスク"),
  
  # Main input and result
  mainPanel(
    # input
    h4("使用上の注意"),
    helpText("このページは古瀬祐気が作成しました。 furusey.kyoto@gmail.com | @ykfrs1217 (Twitter) | https://furuse-lab.mystrikingly.com/"),
    helpText("作成者は、厚生労働省新型コロナクラスター対策班に所属していますが、本ページの内容は同班の公式リリースではありません。"),
    helpText("簡易的なツールであり、多くの不確定な要素が含まれていますので、参考程度に用いてください。"),
    helpText("ソースコード： "),
    helpText("作成・公開 2020年11月10日, 更新 2020年11月11日"),
    ...
    radioButtons("screen", 
                 label = h3("症状スクリーニングを行いますか？（有症状者は参加できないようにしていますか？）"), 
                 choices = list("Yes" = 1, 
                                "No" = 2),
                 selected = 1),
    
    numericInput("target", 
                 label = h3("確率を知りたい、イベント参加者中の感染者数"), 
                 value = 10,
                 min=0),
    ...
    helpText("サーバー代以上に寄附が集まった場合には、ウイルス学・感染症学の研究に使わせていただきたいと思います。"),
    br(),
    br()
...
server <- function(input, output) {
  df <- function() {  
    ...
    if (nowcase >= nowpop) { 
      ...
      result1 <- round((1 - everyescape) * 100, digits = 2)
      if (result1 == 0) {
        result1 <- "<0.01"
      } else if (result1 == 100) {
        result1 <- ">99.99"
      ...
      result3 <- round(result3, digits = 2)
      if (result3 == 0) {
        result3 <- "<0.01"
